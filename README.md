# Cloud SQL + GKE Demo for GenAI

Here is a demo of how to build a basic chatbot API that:

- Uses VertexAI and pgvector to support natural language queries
- Codifies all infrastructure including Cloud SQL instances, PSC wiring, VPC
  networking, and GKE cluster creation using Terraform
- Stores all Terraform state in a remote storage bucket
- Uses Python with asyncpg and FastAPI
- (optional) Deploys a GKE cluster and Cloud SQL instance in two separate
  projects with cross-project access
